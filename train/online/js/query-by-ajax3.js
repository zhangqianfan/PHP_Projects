...
function showResponse(fromStation, toStation, dateString) {
	...
	if (fromStationCode != "" && toStationCode != "" && dateString != "") {
		...
		if (xhr1 == null) {
			...
			var selectDOM = document.getElementById("ticket-type");
			var ticketType = selectDOM.options[selectDOM.selectedIndex].value;
			var url = "http://127.0.0.1/external/train/online/response-ticket-info.php?from=" + fromStationCode + "&to=" + toStationCode + "&date=" + dateString + "&ticket_type=" + ticketType;
...
function stateChange() {
	if (xhr1.status == 200 && xhr1.readyState == 4) {
		if (xhr1.responseText != "") {
			...
			if (xhr1.responseText.match(/<tr/) != null) {
				var num_of_res = xhr1.responseText.match(/<tr class/g).length;
			...
			document.getElementById("resp").innerHTML = xhr1.responseText;
		}
		else alert("响应为空！");
	}
}